...
export function buildAgentSystemPrompt(context: AgentPromptContext): string {
	...
	lines.push(
		...
		`## What you can do`,
		``,
		`You have tools that read and change the real files on this machine — not a copy, not an upload. \`list_files\` and \`search_files\` find things, \`find_symbol\` says where something is declared, \`read_file\` reads one, and \`edit_file\` and \`write_file\` change one.`,
		``,
